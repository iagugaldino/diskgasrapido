...
// ══════════════════════════════════════════════
$utmPaymentMethod = $paymentMethod === 'pix' ? 'pix' : 'credit_card';
...
logWrite('INFO', 'UTMIFY_PENDING_RESPONSE', [
...
if ($utmResult['error']) {
    logWrite('ERROR', 'UTMIFY_CURL', $utmResult['error']);
} elseif ($utmResult['http_code'] !== 200) {
    logWrite('ERROR', 'UTMIFY_FAIL', $utmResult['decoded'] ?? $utmResult['body']);
} else {
    logWrite('INFO', 'UTMIFY_PENDING_OK', "Venda pendente enviada para Utmify. OrderId: {$orderId}");
}
